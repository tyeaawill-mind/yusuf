...
function ChatView({ userName, assistantName }: { userName: string; assistantName: string }) {
  ...
  const { messages, sendMessage, status, input, handleInputChange } = useChat({
    id: "default",
            ...
            {msg.role === "assistant" ? (
              ...
              <div className="max-w-[85%] lg:max-w-[70%]">
                <div className="flex items-center gap-2 mb-1.5 justify-end">
                  ...
                  <div className="flex h-6 w-6 items-center justify-center rounded-full bg-chat-user/20">
                    <User className="h-3 w-3 text-chat-user" />
                  </div>
                </div>
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground">No goals yet. Set your first one.</p>
          </div>
        ) : (
          goals.map((goal) => (
            <div key={goal.id} className="rounded-xl border border-border bg-card p-4">
              ...
              <div className="flex gap-2 mt-3">
                {[0, 25, 50, 75, 100].map((p) => (
                  <button
                    key={p}
                    onClick={() => updateMutation.mutate({ id: goal.id, progress: p })}
...
function InsightsView() {
  ...
  const overdue = todos.filter(
    (t) => t.status !== "completed" && t.target_date && new Date(t.target_date) < new Date(),
  ).length;
  ...
  const cards = [
    ...
    },
    {
      ...
      bg: overdue > 0 ? "bg-destructive/10" : "bg-muted",
    },
    {
      label: "Active Goals",
        ...
        <div className="space-y-2">
          {overdue > 0 && (
            <p className="text-sm text-destructive flex items-center gap-2">
              ...
              You have {overdue} overdue task{overdue > 1 ? "s" : ""}. Consider reprioritizing or adjusting deadlines.
            </p>
          )}
          {pending === 0 && total > 0 && (
          ...
          {memories.length === 0 && (
            <p className="text-sm text-muted-foreground flex items-center gap-2">
              ...
              Aria hasn't learned much about you yet. Share your preferences and priorities in chat.
            </p>
          )}
          {pending > 0 && overdue === 0 && (
            <p className="text-sm text-muted-foreground flex items-center gap-2">
              <ArrowRight className="h-3.5 w-3.5" />
              {pending} task{pending > 1 ? "s" : ""} pending. You're on track — keep going.
            </p>
          )}
        </div>
      </div>
