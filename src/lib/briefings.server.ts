...
export async function generateBriefingItems(prefs: Prefs): Promise<{ intro: string; items: BriefingItem[] }> {
  ...
  const system = `You are Yusuf, a personal news editor for the user. You are reviewing Bangladeshi news coverage focused on corruption, money laundering, and financial irregularities. Group near-duplicate articles. Keep only stories that are clearly on-topic. Rank by importance/freshness. Cap at ${prefs.max_items} items. ${langInstruction} Each summary must be EXACTLY three sentences. Pick an image_url from the article's IMAGE field when present, otherwise leave it null. Never invent facts not present in the article content. Output STRICT JSON only.`;
